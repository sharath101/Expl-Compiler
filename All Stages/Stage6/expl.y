...
%type <no> expr NUM TEXT ID Body END BEGIN1 Slist stmt input output asgn ifstmt whilestmt BREAK CONTINUE  
	  ...
	  <ts> Tname Type INT STR TID TypeDef TypeDefList
...
Program : 	TypeDefBlock GdeclBlock FdefBlock MainBlock		{printf("Program done\n");}
...
output 	: WRITE '(' expr ')'       	{$$ = makenode("write",2,NULL,NULL,$3,NULL,NULL);}
		| WRITE '(' var ')'         {/*if($3->type!=tint || $3->type!=tstr) {printf("Can only write integer/String\n");exit(1);}*/$$ = makenode("write",2,NULL,NULL,$3,NULL,NULL);};
...
expr    : expr PLUS expr		{$$ = makenode("+",4,tint,NULL,$1,NULL,$3);}
		...
		| var MUL var			{$$ = makenode("*",4,tint,NULL,$1,NULL,$3);}
	 	...
	 	| var '(' ')'           {$$ = makenode($1->varname,10,$1->type,NULL,NULL,NULL,NULL);}
	 	| var '(' ArgList ')'	{$$ = makenode($1->varname,10,$1->type,$3,NULL,NULL,NULL);}
	 	| var '(' var ')'		{$$ = makenode($1->varname,10,$1->type,$3,NULL,NULL,NULL);}
	 	| var '(' expr ')'		{$$ = makenode($1->varname,10,$1->type,$3,NULL,NULL,NULL);}
	 	| NUM					{$$ = $1;}
	 	| TEXT                 	{$$ = $1;}
	 	;
